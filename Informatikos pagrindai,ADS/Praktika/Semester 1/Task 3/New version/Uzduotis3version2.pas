...
var choice,choice1,value,pasirinkimas,kiekis:integer;
inFileName,outFileName,textas:string;
d,rez:text;
label pradzia;
...
begin
  writeln('Ar norite ivesti duomenis klaviatura?');
  writeln('(Sutikimui iveskite "1", priesingu atveju bus nuskaitomi is failo)');
  readln(pasirinkimas);
  if pasirinkimas=1 then begin
    writeln('kiek skaiciu norite suvesti?  (max 100)');
    readln(kiekis);
    for i:=1 to kiekis do begin
      ...
      begin
        ...
        until ((IOResult = 0) AND ((length(inFileName)>1))AND(validName(inFileName)) );
...
var
    ...
    i,j,words,numberOfWowels:integer;
...
begin
  ...
  writeln('(Sutikimui iveskite "1", priesingu atveju bus nuskaitomi is failo)');
  readln(pasirinkimas);
  if NOT(pasirinkimas=1) then begin
    repeat
    ...
    until ((IOResult = 0) AND ((length(inFileName)>1))AND(validName(inFileName)=false));
  ...
  if pasirinkimas=1 then begin
    ...
    repeat
        repeat
              ...
              // char1(naujas) char2(pries tai buves)
            c1:=textas[i];
            if (NOT letter(c1)) AND (letter(c2)) then i:=i+1;
            c2:=c1;
            //if i=length(textas) then break;
...
///isvedimas
  for j:=1 to words do begin
    ...
    if (word[j].startingLetter = desiredLetter) AND (findWowels(word[j].inWord) = numberOfWowels) then begin
        if NOT (pasirinkimas=1) then writeln(rez,word[j].inWord) else
        writeln(word[j].inWord);
        end;
  end;
  if NOT pasirinkimas=1 then close(rez);
